...
var CurrentProcessHandle windows.Handle = windows.InvalidHandle

func Demo() {
	pid := uintptr(21456)

	observedPid := map[uintptr]bool{pid: true}

	start := time.Now().UnixNano()
	maps := GetHandlesForProcess(observedPid)

	t := time.Now().UnixNano() - start
	fmt.Printf("Get all handle comsumed %d ns\n", t)

	fmt.Printf("Len of Map: %v\n", len(maps))
	fmt.Printf("Len of pid 21456Z: %v\n", len(maps[pid]))

	maps = TransHandle(maps)
	fmt.Printf("Len of Map atter trans: %v\n", len(maps))
	fmt.Printf("Len of pid 21456  atter trans: %v\n", len(maps[pid]))

	for _, h := range maps[pid] {
		t, err := windows.GetFileType(h)
		if err != nil {
			continue
		}
		fmt.Printf("%v - handle [%v] type: %v\n", pid, h, t)
	}
}
...
func TransHandle(origin PidHandleMap) PidHandleMap {
	...
	for pid, handles := range origin {
		...
		for i := 0; i < len(handles); i++ {
			...
			err := windows.DuplicateHandle(node, handles[i], CurrentProcessHandle,
